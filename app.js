(() => {
      ...
      const APP_REVISION_UI_ = '23.9.99cp';
        ...
        try {
          ...
          if (!registro) {
            registro = await Promise.race([
              navigator.serviceWorker.register('./sw.js?v=23.9.99cp', { updateViaCache: 'none' }),
...
UMA NOVA TENTATIVA DE VISTORIA SERÁ REALIZADA OPORTUNAMENTE.`
      ...
      if ('serviceWorker' in navigator) {
        ...
        window.addEventListener('load', async () => {
          try {
            const reg = await navigator.serviceWorker.register('./sw.js?v=23.9.99cp', { updateViaCache: 'none' });
